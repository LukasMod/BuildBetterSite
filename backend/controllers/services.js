  ...
  try {
    ...
    if (!description || !price || !producers || !title) {
      response.status(400).json({
        message:
          'Nie podano wszystkich wymaganych informacji (opis, producent, cena, tytuł)',
      });

      return;
    }

    const isServiceExist = servicesData.some(
    ...
    response.status(500).json({
      error,
      message:
        'Oops! Coś poszło nie tak, przy metodzie POST w endpointcie /services',
    });
  }
};
...
exports.putService = (request, response, next) => {
  try {
    ...
    if (!description || !id || !price || !producers || !title) {
      response.status(400).json({
        message:
          'Nie podano wszystkich wymaganych informacji (opis, id, cena, producent, tytuł)',
      });

      return;
    }

    const indexServiceToUpdate = servicesData.findIndex(
    ...
    response.status(500).json({
      error,
      message:
        'Oops! Coś poszło nie tak, przy metodzie PUT w endpointcie /services',
    });
  }
};
...
exports.deleteService = (request, response, next) => {
  try {
    ...
    console.log(id);
    const indexServiceToDelete = servicesData.findIndex(
      (service) => service.id === id
    );
    ...
    response.status(500).json({
      error,
      message:
        'Oops! Coś poszło nie tak, przy metodzie DELETE w endpointcie /services/:id',
    });
  }
};
